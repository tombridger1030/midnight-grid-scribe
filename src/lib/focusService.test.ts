...
describe("focusService helpers", () => {
  it("splits a session across midnight boundaries", () => {
    const session = makeSession(
      "cross-midnight",
      "2026-04-07T23:15:00.000Z",
      "2026-04-08T02:45:00.000Z",
    );
  ...
  it("includes live active session time in the current day summary", () => {
    ...
    expect(summary?.firstStartedAt).toBe("2026-04-08T08:00:00.000Z");
    expect(summary?.lastEndedAt).toBe("2026-04-08T10:30:00.000Z");
  });
  ...
  it("aggregates multiple sessions within the same day", () => {
    ...
    expect(summary?.firstStartedAt).toBe("2026-04-08T07:00:00.000Z");
    expect(summary?.lastEndedAt).toBe("2026-04-08T18:00:00.000Z");
  });
  ...
  it("builds timing insights for recent days", () => {
    ...
    expect(insights.earliestStartMinutesThisWeek).toBe(390);
