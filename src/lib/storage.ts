...
export const predefinedMetrics: MetricData[] = [
  ...
  {
    id: "jiuJitsuSessions",
    name: "Jiu-Jitsu Sessions",
    type: "number",
    values: {},
  },
  {
    id: "weightliftingSessions",
    name: "Weightlifting Sessions",
    type: "number",
    values: {},
  },
  {
...
export const loadData = (): TrackerData => {
  try {
    ...
    const missingMetrics = predefinedMetrics.filter(
      (metric) => !existingMetricIds.includes(metric.id),
    );
...
export const exportDataCSV = (): void => {
  ...
  data.metrics.forEach((metric) => {
    ...
    data.dates.forEach((date) => {
      const value =
        metric.values[date] !== undefined ? metric.values[date] : "";
...
export async function saveMetrics(trackerData: TrackerData): Promise<void> {
  try {
    ...
    console.log(
      "📊 Metrics to sync:",
      trackerData.dates.length,
      "days,",
      trackerData.metrics.length,
      "metrics",
    );
    ...
    const entries = trackerData.dates
      .map((date) => {
    ...
    const { data, error } = await supabase.from("metrics").upsert(entries, {
...
export async function loadRoadmapData(): Promise<RoadmapData | null> {
  try {
    ...
    if (goalsError || targetsError) {
      console.log(
        "Error loading roadmap data from Supabase, falling back to localStorage",
      );
      return null;
    }
...
export async function saveRoadmapData(roadmapData: RoadmapData): Promise<void> {
  try {
    ...
    if (targetsError) {
      throw new Error(
        `Failed to save monthly targets: ${targetsError.message}`,
      );
    }
...
export function calculateGoalProgress(
  goal: Pick<Goal, "monthly" | "yearlyTarget" | "isNumeric">,
): { currentTotal: number; progressPct: number } {
  if (!goal.isNumeric) {
    // For non-numeric goals, just return 0s
    return { currentTotal: 0, progressPct: 0 };
  }

  const monthlyValues = Object.values(goal.monthly).filter(
    (val) => val !== undefined && val !== null,
  );
  const currentTotal = monthlyValues.reduce((sum, val) => sum + (val || 0), 0);
  const progressPct =
    goal.yearlyTarget > 0 ? Math.min(1, currentTotal / goal.yearlyTarget) : 0;
...
export async function loadCashConsoleData(): Promise<CashConsoleData> {
  ...
  try {
    ...
    if (!data) {
      console.log(
        "No cash console data in Supabase yet, checking localStorage...",
      );
      const raw = localStorage.getItem("noctisium-cash-console");
      if (raw) {
...
export async function saveCashConsoleData(
  data: CashConsoleData,
): Promise<void> {
  const userId = getCurrentUserId();
  ...
  try {
    // Save to Supabase
    const { error } = await supabase.from("cash_console").upsert(
      {
        user_id: userId,
        data: data as any,
...
export function loadGoalsData(): GoalsData {
  try {
    ...
    if (stored) {
      ...
      const needsMigration = data.goals.some(
        (goal) =>
          !goal.monthlyTargets || Object.keys(goal.monthlyTargets).length === 0,
      );

      if (needsMigration) {
        console.log(
          "Goals data needs migration to include monthly targets, clearing...",
        );
  ...
  const seedGoals: Goal[] = [
    {
      ...
      monthlyTargets: {
        Jun: {
          target: 0,
          description: "0% - MVP in users' hands, no paid plans yet",
        },
    ...
    },
    {
      ...
      monthlyTargets: {
        ...
        Aug: {
          target: 60,
          description: "60 mat sessions logged (fundamentals mastery)",
        },
        Dec: {
          target: 100,
          description: "100 sessions + ≥1 competition podium",
        },
      },
      currentTotal: 0,
      progressPct: 0,
 ...
 */
export function updateGoalMonthly(
  goalId: string,
  month: Month,
  value: number | null,
): GoalsData {
...
export async function loadGoalsFromSupabase(): Promise<GoalsData> {
  try {
    ...
    if (error) {
      console.log(
        "Error loading goals from Supabase, falling back to localStorage:",
        error,
      );
 ...
 */
export async function updateGoalMonthlySupabase(
  goalId: string,
  month: Month,
  value: number | null,
): Promise<GoalsData> {
  try {
    ...
    if (updateError) {
      throw new Error(
        `Failed to update goal monthly value: ${updateError.message}`,
      );
    }
 ...
 */
export function updateNetWorthFromInvestments(
  totalInvestmentsCAD: number,
): GoalsData {
  ...
  try {
    ...
    try {
      ...
      if (kanbanDataStored) {
        ...
        console.log(
          "✓ Synced kanban data:",
          Object.keys(kanbanData.tasks).length,
          "tasks",
        );
      } else {
...
export async function loadFinancialMetrics(): Promise<{
  mrr: number;
  netWorth: number;
}> {
  try {
...
export async function saveRoadmapDataToSupabase(
...
export async function testSupabaseConnection(): Promise<{
  ...
  try {
    ...
    const failedTables = Object.entries(results).filter(
...
export async function syncAllDataToSupabaseWithTest(): Promise<{
  success: boolean;
  message: string;
}> {
  try {
...
export async function saveContentItemWithMetrics(
  ...
  if (!ready) {
    throw new Error(
      "Content tables are not ready. Run migration 013_content_tables.sql.",
    );
  }
  // Insert content item
  ...
  if (metrics) {
    ...
    if (
      ...
      m.retention_ratio = Math.max(
        ...
        Math.min(
          1,
          Number(metrics.average_watch_time_seconds) /
            Number(item.video_length_seconds),
        ),
      );
    }

    if (
      m.shares_per_view == null &&
  ...
  if (metrics && Object.keys(metrics).length > 0) {
    ...
    if (existingMetrics) {
      ...
      if (updateError) {
        ...
        throw new Error(
          `Failed to update content metrics: ${updateError.message}`,
        );
      }
      ...
      if (insertError) {
        ...
        throw new Error(
          `Failed to insert new content metrics: ${insertError.message}`,
        );
      }
...
export async function loadRecentContent(
  ...
  try {
    const { data, error } = await supabase
      .from("content_items")
      .select(
        "id, platform, format, account_handle, title, published_at, tags, url",
      )
      .eq("user_id", userId)
    ...
    if (items.length === 0) {
      console.log(
        "💡 No content found. Make sure content_items have user_id = " + userId,
      );
    }
...
export async function loadWeeklyContentDetail(
  weekKey: string,
): Promise<WeeklyContentDetail> {
  ...
  if (itemsErr)
    throw new Error(`Failed to load weekly content: ${itemsErr.message}`);
  ...
  if (metricsErr)
    throw new Error(`Failed to load weekly metrics: ${metricsErr.message}`);
  ...
  // Calculate summary
  const totalViews = itemsWithMetrics.reduce(
    (sum, item) => sum + (item.views || 0),
    0,
  );
  const totalFollows = itemsWithMetrics.reduce(
    (sum, item) => sum + (item.follows || 0),
    0,
  );
...
export async function loadWeeklyContentSummary(
  weeks: string[],
): Promise<WeeklyContentSummary[]> {
  ...
  (metrics || []).forEach((m: any) => {
    if (!metricsByContent.has(m.content_id))
      metricsByContent.set(m.content_id, m);
  });
  ...
  (items || []).forEach((it: any) => {
    ...
    if (m) {
      ...
      if (!isNaN(r)) {
        ...
        current.avg_retention = Number(
          ((current.avg_retention || 0) + r) as any,
        );
      }
    }
  });
  ...
  weeks.forEach((w) => {
    ...
    if (cur.avg_retention != null && cur.videos_published > 0) {
      cur.avg_retention = Number(
        (cur.avg_retention / cur.videos_published).toFixed(3),
      );
    }
  });
 ...
 */
export async function loadKanbanFromSupabase(
  boardId: string = "echo",
): Promise<KanbanData | null> {
  try {
    ...
    tasksData.forEach((task) => {
      ...
      const deletedAt =
        task.deleted_at !== undefined ? task.deleted_at : undefined;
 ...
 */
export async function saveKanbanToSupabase(
  ...
  try {
    ...
    // Ensure board exists
    const { error: boardError } = await supabase.from("kanban_boards").upsert(
      {
        user_id: getCurrentUserId(),
        board_id: boardId,
    ...
    try {
      ...
      if (fetchError) {
        console.warn(
          "⚠️ Warning: Failed to fetch existing tasks for cleanup:",
          fetchError,
        );
      } else if (existingTasks && existingTasks.length > 0) {
        const currentTaskIds = Object.keys(kanbanData.tasks).filter(
          (taskId) => !kanbanData.tasks[taskId].isDeleted,
        );
          ...
          for (const taskId of tasksToDelete) {
            try {
              ...
              if (deleteError) {
                ...
                console.warn(
                  `⚠️ Soft delete failed for ${taskId}, trying hard delete:`,
                  deleteError,
                );
                ...
                if (hardDeleteError) {
                  console.warn(
                    `⚠️ Failed to delete orphaned task ${taskId}:`,
                    hardDeleteError,
                  );
                }
              }
 ...
 */
export async function deleteKanbanTaskFromSupabase(
  taskId: string,
  boardId: string = "echo",
): Promise<void> {
  try {
    ...
    if (softDeleteError) {
      ...
      console.warn(
        `Soft delete failed for ${taskId}, trying hard delete:`,
        softDeleteError,
      );
 ...
 */
export async function moveKanbanTaskInSupabase(
  taskId: string,
  newColumnId: string,
  boardId: string = "echo",
): Promise<void> {
 ...
 */
export async function restoreKanbanTaskFromSupabase(
  taskId: string,
  boardId: string = "echo",
): Promise<void> {
  try {
 ...
 */
export async function getDeletedKanbanTasks(
  boardId: string = "echo",
): Promise<KanbanTask[]> {
  try {
    const { data: tasksData, error: tasksError } = await supabase
 ...
 */
export async function verifySyncFunctionality(): Promise<{
  success: boolean;
  message: string;
  ...
  try {
    ...
    const originalData = {
      ...
      kanban: localStorage.getItem("noctisium-kanban")
        ? JSON.parse(localStorage.getItem("noctisium-kanban")!)
        : null,
      sprints: localStorage.getItem("noctisium-sprint-start-date"),
      roadmaps: localStorage.getItem("noctisium-roadmaps")
        ? JSON.parse(localStorage.getItem("noctisium-roadmaps")!)
        : null,
    };
    ...
    verificationResults.backup = {
      ...
      kanbanTasks: originalData.kanban
        ? Object.keys(originalData.kanban.tasks).length
        : 0,
    ...
    // Validate metrics
    if (
      downloadedData.metrics.dates.length !== originalData.metrics.dates.length
    ) {
      validationErrors.push(
        `Metrics dates mismatch: ${downloadedData.metrics.dates.length} vs ${originalData.metrics.dates.length}`,
      );
    }

    if (
      downloadedData.metrics.metrics.length !==
      originalData.metrics.metrics.length
    ) {
      validationErrors.push(
        `Metrics count mismatch: ${downloadedData.metrics.metrics.length} vs ${originalData.metrics.metrics.length}`,
      );
    }
    ...
    if (downloadedData.goals.goals.length !== originalData.goals.goals.length) {
      validationErrors.push(
        `Goals count mismatch: ${downloadedData.goals.goals.length} vs ${originalData.goals.goals.length}`,
      );
    }

    // Validate kanban
    const originalKanbanTasks = originalData.kanban
      ? Object.keys(originalData.kanban.tasks).length
      : 0;
    const downloadedKanbanTasks = downloadedData.kanban
      ? Object.keys(downloadedData.kanban.tasks).length
      : 0;
    if (downloadedKanbanTasks !== originalKanbanTasks) {
      validationErrors.push(
        `Kanban tasks mismatch: ${downloadedKanbanTasks} vs ${originalKanbanTasks}`,
      );
    }
    ...
    return {
      success: true,
      message:
        "Complete sync verification successful! All data types synced correctly.",
      details: verificationResults,
    };
  } catch (error) {
 ...
 */
export async function validateDataMigration(): Promise<{
  success: boolean;
  message: string;
  ...
  try {
    ...
    const localFinancial = JSON.parse(
      localStorage.getItem("noctisium-financial-data") || "{}",
    );
    const localKanban = localStorage.getItem("noctisium-kanban")
      ? JSON.parse(localStorage.getItem("noctisium-kanban")!)
      : null;
...
export function deleteAvoidanceItem(itemId: string): void {
  const data = loadNoctisiumData();
  data.avoidanceItems = data.avoidanceItems.filter(
    (item) => item.id !== itemId,
  );
  saveNoctisiumData(data);
}
...
export function startDeepWorkSession(priority: string): DeepWorkSession {
  ...
  if (
    !data.currentSliceId ||
    !data.workSlices.find((s) => s.id === data.currentSliceId && s.isActive)
  ) {
 ...
 */
export async function handleContentCreation(
  title: string,
  publishedAt: string,
  url?: string,
  platform?: string,
  ...
  try {
    ...
    console.log(
      `📱 Content ship created: ${title} (${platform || "unknown platform"})`,
    );
  } catch (error) {
 ...
 */
export function logContentShip(
  title: string,
  url?: string,
  platform?: string,
): ShipRecord {
  const description = platform ? `📱 ${title} (${platform})` : `📱 ${title}`;
...
export function logShip(
  ...
  const sliceId =
    data.currentSliceId ||
    `slice-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`;
  ...
  if (slice?.firstDeepWorkStart) {
    cycleTimeMinutes = Math.round(
      (Date.now() - new Date(slice.firstDeepWorkStart).getTime()) / 60000,
    );
  ...
  data.alerts = data.alerts.filter(
    (alert) => alert.type !== "no_ship" || !alert.isActive,
  );
...
export function getTimeSinceLastShip(): number {
  ...
  const lastShip = data.ships.sort(
    (a, b) => new Date(b.timestamp).getTime() - new Date(a.timestamp).getTime(),
  )[0];
  return (
    (Date.now() - new Date(lastShip.timestamp).getTime()) / (1000 * 60 * 60)
  );
}
 ...
 */
export function setWeeklyConstraint(
  constraint: string,
  reason?: string,
): WeeklyConstraint {
  const data = loadNoctisiumData();
...
export function getCurrentWeeklyConstraint(): WeeklyConstraint | null {
  ...
  return (
    data.weeklyConstraints.find(
      (c) => c.weekStart === weekStart && c.isActive,
    ) || null
  );
}
 ...
 * Update runway data
 */
export function updateRunwayData(
  totalBalance: number,
  monthlyBurn: number,
): RunwayData {
  const data = loadNoctisiumData();
  ...
  const monthsRemaining =
    monthlyBurn > 0 ? Math.floor(totalBalance / monthlyBurn) : 999;
  ...
  const existingAlert = data.alerts.find(
    (a) => a.type === "low_runway" && a.isActive,
  );
  if (monthsRemaining < 12) {
    if (!existingAlert) {
      data.alerts.push({
        ...
        createdAt: new Date().toISOString(),
        suggestedActions:
          runway.suggestedCuts?.concat(runway.suggestedIncomeTargets || []) ||
          [],
...
export function checkAndCreateAlerts(): NoctisiumAlert[] {
  ...
  const existingNoShipAlert = data.alerts.find(
    (a) => a.type === "no_ship" && a.isActive,
  );
