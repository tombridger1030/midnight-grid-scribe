...
import { Plus, ChevronDown, ChevronRight, Trash2, TrendingDown, Edit3, Check, X } from 'lucide-react';
...
export const ExpensesTab: React.FC<ExpensesTabProps> = ({
  ...
  const runwayMonths = avgMonthlyBurn > 0 ? cashReserveCAD / avgMonthlyBurn : Infinity;
  ...
  const getRunwayStatus = () => {
    if (runwayMonths === Infinity) return { label: 'No burn data', color: 'text-terminal-accent/60' };
  ...
  return (
    <div className="space-y-6">
      ...
      >
            ...
            <div className="flex items-baseline gap-3">
              {isEditingReserve ? (
                ...
                >
                  ...
                  <Edit3 size={16} className="text-terminal-accent/40 
        ...
        <div className="h-2 bg-terminal-accent/20 rounded-full overflow-hidden mb-3">
          <motion.div
            className={cn(
              "h-full rounded-full",
              runwayMonths > 12 ? "bg-[#5FE3B3]" :
              runwayMonths > 6 ? "bg-[#FFD700]" :
              runwayMonths > 3 ? "bg-[#FFA500]" : "bg-[#FF6B6B]"
            )}
            initial={{ width: 0 }}
      ...
      >
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            <span className="absolute left-3 top-1/2 -translate-y-1/2 text-terminal-accent/60">$</span>
            <input
              ref={amountRef}
              type="number"
              step="0.01"
      ...
      <div className="space-y-3">
        {Array.from(groupedExpenses.entries()).map(([monthKey, monthExpenses], index) => {
          ...
          return (
            ...
            >
              ...
              >
                <div className="flex items-center gap-2">
                  {isExpanded ? (
                    <ChevronDown size={16} className="text-terminal-accent/60" />
                  ) : (
                    <ChevronRight size={16} className="text-terminal-accent/60" />
                  )}
