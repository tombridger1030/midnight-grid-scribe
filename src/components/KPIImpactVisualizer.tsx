...
const KPIImpactVisualizer: React.FC<KPIImpactVisualizerProps> = ({
  ...
  const contributions = useMemo(
    () => WEEKLY_KPI_SKILL_MAPPING[skill.id] || [],
    [skill.id],
  );
  const [progressByKpi, setProgressByKpi] = useState<Record<string, number>>(
    {},
  );
  ...
  useEffect(() => {
    ...
    const loadProgress = async () => {
      ...
      const progressEntries = await Promise.all(
        contributions.map(async ({ kpiId }) => {
          const currentValue = currentWeekKPIs[kpiId] || 0;
          const progress = await calculateKPIProgress(kpiId, currentValue);
          return [kpiId, progress] as const;
        }),
      );

      if (isMounted) {
        setProgressByKpi(Object.fromEntries(progressEntries));
      }
    };

    loadProgress().catch((error) => {
      console.error("Failed to resolve KPI progress for visualizer:", error);
      if (isMounted) setProgressByKpi({});
    });

    return () => {
      isMounted = false;
    };
  }, [contributions, currentWeekKPIs]);

  // Calculate impact data
  const impactData = useMemo(
    () =>
      contributions
        .map(({ kpiId, weight, formula }) => {
          const kpiDefinition = WEEKLY_KPI_DEFINITIONS.find(
            (def) => def.id === kpiId,
          );
          if (!kpiDefinition) return null;

          const currentValue = currentWeekKPIs[kpiId] || 0;
          const progress = progressByKpi[kpiId] ?? 0;
        ...
        .filter(Boolean),
    [contributions, currentWeekKPIs, progressByKpi],
  );

  // Calculate total potential impact
  const totalPotentialImpact = contributions.reduce(
    (sum, { weight }) => sum + weight,
    0,
  );
  const actualImpact = impactData.reduce(
    (sum, item) => sum + (item?.impactScore || 0),
    0,
  );
  const impactEfficiency =
    totalPotentialImpact > 0 ? (actualImpact / totalPotentialImpact) * 100 : 0;
    ...
    <div className="space-y-6">
      ...
      <div className="p-4 border border-terminal-accent/30 bg-terminal-bg/20">
        ...
        <div className="grid grid-cols-3 gap-4 mb-4">
          ...
          <div className="text-center">
            <div className="text-2xl font-bold text-terminal-accent/70">
        ...
        <Progress
          ...
          className="h-2"
          style={
            { "--progress-background": skill.color } as React.CSSProperties
          }
        />
      </div>
      ...
      <div className="space-y-3">
        <h4 className="text-sm font-medium text-terminal-accent">
          KPI Contributions
        </h4>
      ...
      <div className="border border-terminal-accent/30 p-4">
        <h4 className="text-sm font-medium text-terminal-accent mb-3">
          Impact Breakdown
        </h4>
        <div style={{ height }}>
      ...
      <div className="text-xs text-terminal-accent/70 p-3 border border-terminal-accent/20 bg-terminal-bg/10">
        ...
        <div className="space-y-1">
          <div>
            <span className="font-medium">Linear:</span> Direct proportional
            impact (progress × weight)
          </div>
          <div>
            <span className="font-medium">Threshold:</span> Full impact only
            when 80%+ target achieved
          </div>
          <div>
            <span className="font-medium">Exponential:</span> Accelerating
            impact for network effects
          </div>
        </div>
      </div>
    </div>
