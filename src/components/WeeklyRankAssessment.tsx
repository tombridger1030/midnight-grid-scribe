...
import { rankingManager, WeeklyAssessment, RANK_CONFIG } from '@/lib/rankingSystem';
import { getCurrentWeek, getPreviousWeek } from '@/lib/weeklyKpi';
import { userStorage } from '@/lib/userStorage';
import { toast } from 'sonner';
import { Trophy, TrendingUp, TrendingDown, Target, Calendar, Award, AlertTriangle } from 'lucide-react';
...
const WeeklyRankAssessment: React.FC<WeeklyRankAssessmentProps> = ({
  ...
  const handleAssessment = async () => {
    ...
    try {
      ...
      const weeklyAssessment = await rankingManager.assessWeeklyPerformance(targetWeek);
      ...
      if (weeklyAssessment.rank_after !== weeklyAssessment.rank_before) {
        const rankUp = RANK_CONFIG[weeklyAssessment.rank_after].min_rr > RANK_CONFIG[weeklyAssessment.rank_before].min_rr;
        toast.success(
    ...
    return (
      <div className={`border border-terminal-accent/30 p-4 bg-terminal-bg/20 ${className}`}>
        <div className="text-center">
          ...
          <h3 className="text-lg font-bold text-terminal-accent mb-2">Weekly Assessment</h3>
          <p className="text-sm text-terminal-accent/70 mb-4">
            Evaluate your performance for week {targetWeek.split('-W')[1]} and update your rank.
          </p>
          <button
  ...
  if (isAssessing) {
    return (
      <div className={`border border-terminal-accent/30 p-4 bg-terminal-bg/20 ${className}`}>
        <div className="text-center">
          <div className="animate-spin mx-auto mb-3 text-terminal-accent" size={32}>⚡</div>
          <h3 className="text-lg font-bold text-terminal-accent mb-2">Assessing Performance...</h3>
    ...
    <div className={`border border-terminal-accent/30 p-4 bg-terminal-bg/20 ${className}`}>
        ...
        <div className="space-y-3">
          <div className="text-center">
            <div className="text-3xl font-bold mb-1" style={{
              color: assessment.completion_percentage >= 80 ? '#5FE3B3' :
                     assessment.completion_percentage >= 50 ? '#FFD700' : '#FF6B6B'
            }}>
              {assessment.completion_percentage}%
            </div>
            <div className="text-sm text-terminal-accent/70">Weekly Completion</div>
          </div>
          ...
          <div className="flex items-center justify-center gap-2">
            ...
            <div className={`text-lg font-bold font-mono ${
              assessment.rr_change > 0 ? 'text-green-400' :
              assessment.rr_change < 0 ? 'text-red-400' : 'text-terminal-accent/60'
            }`}>
              {assessment.rr_change > 0 ? '+' : ''}{assessment.rr_change} RR
            </div>
          </div>
        </div>
      ...
      {assessment.rank_after !== assessment.rank_before && (
        <div className={`p-3 rounded border mb-4 ${
          RANK_CONFIG[assessment.rank_after].min_rr > RANK_CONFIG[assessment.rank_before].min_rr
          ...
          <div className="text-sm text-terminal-accent/70 mt-1">
            {RANK_CONFIG[assessment.rank_before].name} → {RANK_CONFIG[assessment.rank_after].name}
          </div>
        </div>
      )}
        ...
        <div className="space-y-2 border-t border-terminal-accent/20 pt-3">
          {assessment.kpi_breakdown.map((kpi, index) => (
            <div key={index} className="flex items-center justify-between text-sm">
              ...
              <div className={`font-mono ${kpi.completed ? 'text-green-400' : 'text-red-400'}`}>
