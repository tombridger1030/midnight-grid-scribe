...
export function useBooks(weekKey: string): UseBooksReturn {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err : new Error("Failed to load book data"),
      );
    } finally {
      setIsLoading(false);
    }
  ...
  }, [weeklyPagesRead, user?.id, weekKey]);
  ...
  const updateProgress = useCallback(
    async (bookId: string, update: ProgressUpdate) => {
      if (!user?.id) return;
      ...
      try {
        ...
        // Award XP for meaningful progress (>10 pages or >5% for audiobooks)
        const meaningfulProgress =
          book.book_type === "audiobook" ? percentDelta >= 5 : pagesRead >= 10;
        if (meaningfulProgress) {
          useProgressionStore.getState().onKPIEntry("book_progress");
        }
      } catch (err) {
  ...
  const completeBook = useCallback(
    async (bookId: string) => {
      ...
      try {
        ...
        useProgressionStore.getState().onKPIEntry("book_complete");
      } catch (err) {
  ...
  const pauseBook = useCallback(
    async (bookId: string) => {
      ...
      try {
        ...
        setBooks((prev) =>
          prev.map((b) =>
            b.id === bookId ? { ...b, status: "paused" as BookStatus } : b,
          ),
        );
      } catch (err) {
  ...
  const resumeBook = useCallback(
    async (bookId: string) => {
      ...
      try {
        ...
        setBooks((prev) =>
          prev.map((b) =>
            b.id === bookId ? { ...b, status: "reading" as BookStatus } : b,
          ),
        );
      } catch (err) {
